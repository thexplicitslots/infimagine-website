...
async function listGalleryItems() {
  ...
  return { configured: true, provider: "neon", items: await Promise.all(rows.map(mapRecord)) };
}
...
async function createGalleryItem(payload = {}) {
  ...
  return { configured: true, provider: "neon", item: await mapRecord(rows[0]) };
}
...
async function deleteGalleryItem(id) {
  ...
  return { configured: true, provider: "neon", item: record ? await mapRecord(record) : null };
}
